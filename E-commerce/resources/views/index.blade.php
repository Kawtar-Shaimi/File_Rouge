...
<section class="hero text-white text-center py-20">
    <div class="container mx-auto px-4">
        ...
        <a href="{{ route('products') }}" class="bg-white text-blue-600 font-bold py-3 px-6 rounded-lg shadow-lg hover:bg-gray-200">Voir les produits</a>
    </div>
</section>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        @if ($products->count() > 0)
            @foreach ($products as $product)
                <div id="product-{{ $product->id }}" class="bg-white rounded-lg shadow-lg p-6 product-card">
                    <a href="{{ route('products.show', $product) }}">
                        <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="rounded-md mb-4 w-full h-48 object-cover">
                        <h3 class="text-xl font-bold mb-2 text-blue-600">{{ $product->name }}</h3>
                        <p class="text-gray-600 mb-4">{{ Str::limit($product->description, 15) }}</p>
        ...
        @else
            <div class="col-span-1 md:col-span-2 lg:col-span-4">
                <p class="text-red-500 text-4xl font-bold text-center py-32">No Product Yet</p>
            </div>
        @endif
