...
@section('content')
    ...
    <div class="container mx-auto p-6 max-w-5xl">
        <h2 class="text-2xl font-bold text-gray-800 mt-10 mb-5">Book Details</h2>
        <div id="book-{{ $book->uuid }}" class="bg-white p-6 rounded-lg shadow-lg flex flex-col md:flex-row">
            ...
            <div class="w-full md:w-1/2 md:pl-6">
                ...
                <div class="mb-4">
                    ...
                    @if ($book->stock > 0)
                        <span class="text-green-600 font-semibold ml-4">✔ {{ $book->stock }} In stock</span>
                    @else
                        <span class="text-red-600 font-semibold ml-4">✘ Out of stock</span>
                    @endif
                </div>
                        ...
                        <div id="actions" class="mt-4 flex items-end">
                            @if ($book->isInCart)
                                ...
                                <input type="text" id="quantity-{{ $book->uuid }}" name="quantity"
                                    value="{{ $book->bookQuantity }}" class="w-3/5 text-center p-2 bg-gray-100 text-gray-900"
                                    readonly disabled>
                ...
                <div id="review-form" @if (!$book->isReviewed) class="border-b border-b-black" @endif>
                    @if (!$book->isReviewed)
                        ...
                        <div class="flex items-center mb-4 space-x-1" id="review-rating">
                            ...
                            <p class="text-center text-lg font-semibold text-gray-700 !ml-5"><span id="rating-value">0</span> /
                                5</p>
                        </div>
                        ...
                        <div class="mb-4">
                            <div class="flex justify-center">
                                <input id="review-content" type="text"
                                    class="w-4/5 p-3 border border-gray-300 rounded-s-lg focus:ring focus:ring-purple-400"
                                    placeholder="Write your review here...">
                                <button id="submit-review"
                                    class="w-1/5 bg-purple-500 text-white font-bold py-3 rounded-e-lg hover:bg-purple-600 transition duration-200"
                                    ...
                                    <div id="actions">
                                        <div class="mt-4 flex items-center justify-end">
                                            <button id="showUpdateReviewFormBtn-{{ $review->uuid }}"
                                                class="w-4/5 bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-s-lg"
                                                onclick="showUpdateReviewForm('{{ $book->uuid }}', '{{ $review->uuid }}', '{{ $review->content }}', {{ $review->rate }})">
                                                update
                                            </button>
                                            <button id="deleteReviewBtn-{{ $review->uuid }}"
                                                class="bg-red-500 hover:bg-red-600 text-red-500 py-2 px-4 rounded-e-lg"
                                                onclick="deleteReview('{{ $review->uuid }}', '{{ $book->uuid }}')">🗑</button>
                                        </div>
                                    </div>
                                @endif
                            @endauth
