...
<div class="container mx-auto p-6 max-w-5xl">
    ...
    <div id="book-{{ $book->id }}" class="bg-white p-6 rounded-lg shadow-lg flex flex-col md:flex-row">
                ...
                @if(!$book->isReviewed)
                    ...
                    <div class="mb-4">
                        <div class="flex justify-center">
                            ...
                            <button id="submit-review" class="w-1/5 bg-purple-500 text-white font-bold py-3 rounded-e-lg hover:bg-purple-600 transition duration-200" onclick="addReview({{ $book->id }})">
                                Send
                            </button>
                        </div>
                            ...
                            @if ($review->client_id === Auth::guard('client')->id())
                                <div id="actions">
                                    <div class="mt-4 flex items-center justify-end">
                                        <button id="showUpdateReviewFormBtn-{{ $review->id }}" class="w-4/5 bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-s-lg" onclick="showUpdateReviewForm({{ $book->id }}, {{ $review->id }}, '{{ $review->content }}', {{ $review->rate }})">
                                            update
                                        </button>
                                        <button id="deleteReviewBtn-{{ $review->id }}" class="bg-red-500 hover:bg-red-600 text-red-500 py-2 px-4 rounded-e-lg" onclick="deleteReview({{ $review->id }}, {{ $book->id }})">🗑</button>
                                    </div>
                                </div>
                            @endif
                        @endauth
