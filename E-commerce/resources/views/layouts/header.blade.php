<header class="bg-gray-900 text-gray-200 shadow-lg">
    <div class="container mx-auto px-4 py-4 flex justify-between items-center">
        <!-- Logo -->
        <a href="{{ route('home') }}" class="text-2xl font-bold text-teal-400 flex items-center space-x-2">
            <i class="fas fa-book text-teal-400"></i>
            <span>NextPage</span>
        </a>
            ...
            <div class="flex items-center">
                <a href="{{ route('client.index') }}" class="flex items-center space-x-4 px-4 py-2 hover:bg-gray-700">
                    <button class="flex items-center space-x-2">
                        <img src="https://ui-avatars.com/api/?name={{ Auth::guard('client')->user()->name }}&background=2dd4bf&color=fff"
                ...
                <div id="notification" class="relative mx-5">
                    ...
                    <span id="notification-count"
                        class="absolute top-0 -right-1 bg-teal-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-sm cursor-pointer">
                        ...
                        <ul class="py-2">
                            @if ($user->unreadNotifications->count() > 0)
                                ...
                                <li><a href="{{ route('notifications.client') }}"
                                        class="block text-teal-400 px-4 py-1 text-sm hover:underline">View all
                                        notifications</a></li>
                            @else
        ...
        @auth('publisher')
            ...
            <div class="flex items-center">
                <a href="{{ route('publisher.profile') }}" class="flex items-center space-x-4 px-4 py-2 hover:bg-gray-700">
                    <button class="flex items-center space-x-2">
                        <img src="https://ui-avatars.com/api/?name={{ Auth::guard('publisher')->user()->name }}&background=2dd4bf&color=fff"
        ...
        @auth('admin')
            ...
            <div class="flex items-center">
                <a href="{{ route('admin.profile') }}" class="flex items-center space-x-4 px-4 py-2 hover:bg-gray-700">
                    <button class="flex items-center space-x-2">
                        <img src="https://ui-avatars.com/api/?name={{ Auth::guard('admin')->user()->name }}&background=2dd4bf&color=fff"
