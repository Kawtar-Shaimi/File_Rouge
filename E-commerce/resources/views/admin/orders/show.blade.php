...
@section('head')
    @vite(['resources/css/app.css', 'resources/js/app.js', 'resources/js/admin/orders/order.js'])
@endsection
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        <table class="w-full border-collapse">
                            ...
                            <tr>
                                ...
                                <td class="p-3 border underline italic hover:text-blue-400"><a
                                        href="{{ route('admin.payments.show', $order->payment->uuid) }}">#{{ $order->payment->uuid }}
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="p-3 border">
                                    ...
                                    @endif
                                    <form id="delete-form" action="{{ route('admin.orders.delete', $order->uuid) }}"
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        <table class="w-full border-collapse">
                            <tr>
                                ...
                                <td class="p-3 border underline italic hover:text-blue-400"><a
                                        href="{{ route('admin.users.show', $order->client->uuid) }}">#{{ $order->client->uuid }}
                                </td>
                            </tr>
                            <tr>
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        <table class="w-full border-collapse">
                            ...
                            <tbody>
                                @if ($order->orderBooks->count() > 0)
                                    @foreach ($order->orderBooks as $orderBook)
                                        <tr class="text-center">
                                            <td class="p-3 border underline italic hover:text-blue-400"><a
                                                    href="{{ route('admin.books.show', $orderBook->book->uuid) }}">#{{ $orderBook->book->uuid }}</a>
                                            </td>
                                            <td class="p-3 border">{{ $orderBook->book->name }}</td>
                                            <td class="p-3 border truncate w-40">
                                                {{ Str::limit($orderBook->book->description, 15) }}</td>
                                            <td class="p-3 border text-green-600 font-bold">${{ $orderBook->book->price }}
                                            </td>
                                            ...
                                            <td class="p-3 border">
                                                @if (!$orderBook->is_cancelled)
                                                    @if ($order->status == 'pending')
                                                        <span
                                                            class="bg-yellow-400 text-white px-3 py-1 rounded">{{ $order->status }}</span>
                                                    @elseif ($order->status == 'in shipping')
                                                        <span
                                                            class="bg-blue-400 text-white px-3 py-1 rounded">{{ $order->status }}</span>
                                                    @elseif ($order->status == 'completed')
                                                        <span
                                                            class="bg-green-400 text-white px-3 py-1 rounded">{{ $order->status }}</span>
                                                    @else
                                                        <span
                                                            class="bg-red-400 text-white px-3 py-1 rounded">{{ $order->status }}</span>
                                                    @endif
                                                @else
                                ...
                                @else
                                    <tr>
                                        <td colspan="8" class="text-red-500 text-center py-3 px-6 text-2xl font-bold">No
                                            Books In This Order</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
