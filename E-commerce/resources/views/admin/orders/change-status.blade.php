...
@section('head')
    @vite(['resources/css/app.css', 'resources/js/app.js', 'resources/js/admin/orders/changeStatusInputValidation.js'])
@endsection
                ...
                <div class="bg-white p-6 rounded-lg shadow-md">
                    ...
                    <form id="update-form" action="{{ route('admin.orders.update', $order->uuid) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="status" name="status" class="w-full p-3 border rounded-lg mt-1" required>
                                ...
                                <option value="pending" {{ $order->status == 'pending' ? 'selected' : '' }}
                                    {{ $order->status == 'in shipping' || $order->status == 'completed' || $order->status == 'cancelled' ? 'disabled' : '' }}>
                                    Pending</option>
                                <option value="in shipping" {{ $order->status == 'in shipping' ? 'selected' : '' }}
                                    {{ $order->status == 'completed' || $order->status == 'cancelled' ? 'disabled' : '' }}>
                                    In Shipping</option>
                                <option value="completed" {{ $order->status == 'completed' ? 'selected' : '' }}
                                    {{ $order->status == 'cancelled' ? 'disabled' : '' }}>Completed</option>
                                <option value="cancelled" {{ $order->status == 'cancelled' ? 'selected' : '' }}
                                    {{ $order->status == 'completed' ? 'disabled' : '' }}>Cancelled</option>
                            </select>
                        </div>
                        ...
                        <button id="update-status" type="submit"
                            class="w-full bg-purple-400 text-white p-3 rounded-lg hover:bg-blue-700 transition mt-4">Change
                            Status</button>
                    </form>
                </div>
            </div>
