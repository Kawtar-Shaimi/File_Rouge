        ...
        @if ($users->count() > 0)
            @foreach ($users as $user)
                <tr class="text-center">
                    ...
                    <td class="p-3 border text-gray-500 italic">{{ $user->phone }}</td>
                    ...
                    <td class="p-3 border">
                        <div class="flex justify-center items-center space-x-2">
                            ...
                            <form id="delete-form-{{ $user->uuid }}"
                                action="{{ route('admin.users.delete', $user->uuid) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button id="delete-{{ $user->uuid }}" type="submit"
                                    class="bg-red-500 text-white px-3 py-1 rounded"
                                    onclick="showDeleteConfirmation(event, '{{ $user->uuid }}')">Delete</button>
                            </form>
                        </div>
                    </td>
                </tr>
