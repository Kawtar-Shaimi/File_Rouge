            ...
            <div class="flex items-center justify-between space-x-8 border-b pb-6">
                <div class="flex items-center space-x-4">
                    <img class="w-24 h-24 rounded-full border-4 border-indigo-600 "
                        src="https://ui-avatars.com/api/?name={{ Auth::guard('admin')->user()->name }}&background=000&color=c084fc"
                        alt="{{ Auth::guard('admin')->user()->name }}">
                    <div>
                        <h3 class="text-3xl font-bold text-gray-800">{{ Auth::guard('admin')->user()->name }}</h3>
                        <p class="text-lg text-gray-600">Email : {{ Auth::guard('admin')->user()->email }}</p>
                        <p class="text-lg text-gray-600">Phone : <span
                                class="text-indigo-500">{{ Auth::guard('admin')->user()->phone }}</span></p>
                        <p class="text-gray-600 text-lg font-semibold">Role : <span
                                class="text-indigo-500">{{ Auth::guard('admin')->user()->role }}</span></p>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto bg-gray-100 p-6 rounded-lg shadow-lg">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody>
                                ...
                                @foreach ($orders as $order)
                                    <tr class="border-t hover:bg-indigo-50 text-gray-600">
                                        <td class="py-3 px-4 underline italic hover:text-blue-400"><a
                                                href="{{ route('admin.orders.show', $order->uuid) }}">#{{ $order->uuid }}</a>
                                        </td>
                                        <td class="py-3 px-4 underline italic hover:text-blue-400"><a
                                                href="{{ route('admin.orders.show', $order->uuid) }}">#{{ $order->order_number }}</a>
                                        </td>
                            ...
                            @else
                                <tr class="border-t hover:bg-indigo-50 text-gray-600">
                                    <td colspan="7" class="text-red-500 text-center py-3 px-6 text-2xl font-bold">No
                                        Orders Yet</td>
                                </tr>
                            @endif
                        </tbody>
