    ...
    <div class="container mx-auto p-8">
        <div class="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-xl space-y-6">
            ...
            <div class="flex items-center justify-between space-x-8 border-b pb-6">
                ...
                <a href="{{ route('users.edit', Auth::guard('client')->user()->uuid) }}" class="px-6 py-2 bg-purple-400 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 transition-all duration-300">
                    Modifier le Profil
                </a>

                <a href="{{ route('users.change-password.view', Auth::guard('client')->user()->uuid) }}" class="px-6 py-2 bg-gray-400 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 transition-all duration-300">
                    Reset password
                </a>
            </div>
                        ...
                        <tbody>
                            @if ($orders->count() > 0)
                                ...
                                <tr class="border-t hover:bg-indigo-50 text-gray-600">
                                    ...
                                    <td class="py-3 px-4">
                                        <a href="{{ route('client.order.show', $order->uuid) }}" class="px-4 py-1 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition duration-300">Voir</a>
                                    </td>
                                </tr>
                                @endforeach
                            @else
