        ...
        @if ($orders->count() > 0)
            @foreach ($orders as $order)
                <tr class="text-center">
                    ...
                    <td
                        class="p-3 border font-bold {{ $order->order->payment->status === 'paid' ? 'text-green-600' : ($order->order->payment->status === 'failed' ? 'text-red-600' : 'text-black') }}">
                ...
                <div id="cancel-modal-{{ $order->order->uuid }}"
                    ...
                    <div class="w-2/4 bg-white px-14 py-16 rounded-lg shadow-lg">
                        <form id="cancel-form-{{ $order->order->uuid }}"
                            action="{{ route('publisher.orders.cancel', $order->order->uuid) }}" method="post">
                            @csrf
                            ...
                            <div class="mt-4 flex justify-center items-center space-x-2">
                                ...
                                <button id="cancel-order-{{ $order->order->uuid }}" type="submit"
                                    class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-700 transition"
                                    onclick="showCancelConfirmation(event, '{{ $order->order->uuid }}')">Cancel
                                    Order</button>
                            </div>
                        </form>
                    </div>
